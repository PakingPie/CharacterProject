#ifndef URP_MY_TOON_SHADER_INCLUDED_HLSL
    #define URP_MY_TOON_SHADER_INCLUDED_HLSL
    ...
    float4 frag(v2f i, half facing : VFACE) : SV_Target
    {
        ...
        #ifdef _DISABLE_TRANSCLIPPING
            // No need implementation
        #elif _ENABLE_TRANSCLIPPING
        ...
        _Color = _BaseColor;
            ...
            float4 grediantColor = tex2Dlod(_GrediantMap, float4(TRANSFORM_TEX(uv, _GrediantMap), 0.0, _BlurLevelSGM));
            ...
            float systemShadows = (shadowAttenuation * 0.5) + 0.5 + _TweakSystemShadowsLevel > 0.001 ? (shadowAttenuation * 0.5) + 0.5 + _TweakSystemShadowsLevel : 0.0001;
            grediantColor = grediantColor.r < 0.95 ? grediantColor.r + _TweakShadingGradeMapLevel : 1;
            grediantColor = saturate(grediantColor) * lerp(halfLambert, (halfLambert * saturate(systemShadows)), _BlendSystemShadowsToBase );

            float finalShadowMask = saturate((1.0 + ((grediantColor - (_FirstShadowColorStep - _FirstShadowColorFeather)) * (0.0 - 1.0) ) / (_FirstShadowColorStep - (_FirstShadowColorStep - _FirstShadowColorFeather))));
            baseColor = lerp(baseColor, blendFirstShadowColorWithLightColor, finalShadowMask);
            ...
            float secondShadowMask = saturate((1.0 + ((grediantColor - (_SecondShadowColorStep - _SecondShadowColorFeather)) * (0.0 - 1.0) ) / (_SecondShadowColorStep - (_SecondShadowColorStep - _SecondShadowColorFeather))));
            float3 finalBaseColor = lerp(baseColor, lerp(blendFirstShadowColorWithLightColor, lerp((secondShadowColor.rgb * _SecondShadowTexColor.rgb), ((secondShadowColor.rgb * _SecondShadowTexColor.rgb) * lightColor), _BlendSecondShadowAndLightColor),secondShadowMask), finalShadowMask);
            ...
            highlight = (lerp(SATURATE_IF_SDR((finalBaseColor - tweakHighlightMask)), finalBaseColor, lerp(_IsBlendAddToHighlighColor, 1.0, _SetSpecularColorToHighlightColor)) + lerp(highlight, (highlight * ((1.0 - finalShadowMask) + (finalShadowMask * _TweakHighlightOnShadow))), _SetSpecularColorToHighlightColor));
            ...
            float3 pointLightColor = 0;
                ...
                for(int loopCount = 0; loopCount < pixelLightCount - MAINLIGHT_IS_MAINLIGHT; loopCount++)
                {
                    ...
                    if (iLight != i.mainLightID)
                    {
                        ...
                        _FirstShadowColorStep = saturate(_FirstShadowColorStep + _StepOffset);
                        _SecondShadowColorStep = saturate(_SecondShadowColorStep + _StepOffset);
                        ...
                        lightColor = lerp(lightColor, lerp(lightColor, min(lightColor, additionalLightColor.rgb * _FirstShadowColorStep), notDirectional), _IsFilterHiCutPointLightColor);
                        ...
                        float finalShadowMask = saturate((1.0 + ((grediantColor - (_FirstShadowColorStep - _FirstShadowColorFeather)) * (0.0 - 1.0)) / (_FirstShadowColorStep - (_FirstShadowColorStep - _FirstShadowColorFeather))));
                        float shadeShadowMask = saturate((1.0 + ((grediantColor - (_SecondShadowColorStep - _SecondShadowColorFeather)) * (0.0 - 1.0)) / (_SecondShadowColorStep - (_SecondShadowColorStep - _SecondShadowColorFeather)))); // 1st and 2nd Shades Mask

                        float3 finalColor = lerp(baseColor, lerp(firstShadowColor, lerp(secondShadowColor, secondShadowColor, shadeShadowMask), finalShadowMask), finalShadowMask);
